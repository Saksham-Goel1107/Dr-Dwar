...
export default function HospitalDataDisplay() {
  ...
  const loadHospitalData = async () => {
    try {
      ...
      const uniqueCategories = [...new Set(hospitalData.map(h => h.Hospital_Category).filter(Boolean))];
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {paginatedHospitals.map((hospital) => (
          <div key={hospital.Sr_No} className="bg-white p-4 rounded-lg shadow border">
            <h3 className="text-lg font-semibold mb-2 text-blue-800">
              {hospital.Hospital_Name}
            </h3>
            
            <div className="space-y-1 text-sm">
              <p><span className="font-medium">Category:</span> {hospital.Hospital_Category}</p>
              <p><span className="font-medium">Location:</span> {hospital.District}, {hospital.State}</p>
              <p><span className="font-medium">Address:</span> {hospital.Address_Original_First_Line}</p>
              
              {hospital.Telephone && (
                <p><span className="font-medium">Phone:</span> {hospital.Telephone}</p>
              )}
              
              {hospital.Emergency_Num && (
                <p className="text-red-600"><span className="font-medium">Emergency:</span> {hospital.Emergency_Num}</p>
              )}
              
              {hospital.Total_Num_Beds && (
                <p><span className="font-medium">Beds:</span> {hospital.Total_Num_Beds}</p>
              )}
              
              {hospital.Number_Doctor && (
                <p><span className="font-medium">Doctors:</span> {hospital.Number_Doctor}</p>
              )}
              
              {hospital.Specialties && (
                <p className="text-green-600"><span className="font-medium">Specialties:</span> {hospital.Specialties}</p>
              )}
              
              {hospital.Website && (
                <p>
                  <span className="font-medium">Website:</span> 
                  <a href={hospital.Website} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline ml-1">
                    Visit
                  </a>
                </p>
              )}
