      ...
      <SafeAreaView className="flex-1 bg-gradient-to-br from-gray-50 to-white">
        ...
        <View className="px-6 pt-6 pb-4">
          <View className="flex-row items-center justify-center mb-2">
            <Text className="text-4xl font-bold bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-600 bg-clip-text text-transparent">
              Dr.
            </Text>
            <Text className="text-4xl font-bold text-gray-900 ml-1">
              Dwar
            </Text>
          </View>
          <Text className="text-base text-gray-600 text-center mt-2 font-medium">
        ...
        <View className="flex-1 px-4">
          <Carousel
            ...
            renderItem={({ item, index }) => (
              <View className={`flex-1 items-center justify-center p-8 ${item.color} mx-2 rounded-3xl shadow-xl border border-white/20`}>
                <View className="bg-white rounded-2xl p-8 shadow-2xl mb-8 border border-gray-100">
                  <Image
                    source={item.image}
                    className="w-40 h-40"
                    resizeMode="contain"
                ...
                <Text className={`text-2xl font-bold ${item.accent} text-center mb-4 leading-tight`}>
                  {item.title}
                </Text>
                <Text className="text-base text-gray-700 text-center leading-7 px-2">
                  {item.desc}
                </Text>
              </View>
            )}
        ...
        <View className="flex-row justify-center items-center py-8">
          {data.map((_, index) => (
            <View
              key={index}
              className={`h-3 rounded-full mx-1.5 transition-all duration-500 ${
